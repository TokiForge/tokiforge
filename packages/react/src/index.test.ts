  ...
  return {
    ...
    ThemeRuntime: vi.fn().mockImplementation((class {
      ...
      constructor(config: any) {
        ...
        this.addEventListener.mockImplementation((event: string, callback: () => void) => {
          if (event === 'themechange') {
            listeners.add(callback);
          }
        });
        this.removeEventListener.mockImplementation((event: string, callback: () => void) => {
          if (event === 'themechange') {
...
describe('React Integration', () => {
  ...
  describe('ThemeProvider', () => {
    ...
    it('should initialize ThemeRuntime on mount', () => {
      ...
      expect(instance.applyTheme).toHaveBeenCalled();
    });
