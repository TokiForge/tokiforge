...
export class FigmaSync {
  ...
  async bidirectionalSync(
    ...
    const stateKey = options?.stateKey || this.config.fileKey;
    const strategy = options?.strategy || 'merge';
    const remoteTokens = await this.pullTokens();
    ...
    // Detect what changed
    const remoteChanges = previousState?.remoteHash !== remoteHash ? 
      this.getChangedPaths(previousState ? this.reconstructTokens(previousState.remoteHash) : {}, remoteTokens) : 
      [];
    
    const localChanges = previousState?.localHash !== localHash ? 
      this.getChangedPaths(previousState ? this.reconstructTokens(previousState.localHash) : {}, localTokens) : 
      [];
    ...
    if (strategy === 'local-wins') {
      ...
      merged = this.mergeTokens(localTokens, remoteTokens, (path, local, remote) => {
        conflicts.push(path);
        return options?.onConflict ? options.onConflict(path, local, remote) : remote;
      });
    } else {
      // Default merge
      merged = this.mergeTokens(localTokens, remoteTokens, (path, local, remote) => {
        ...
        return remote; // Remote wins in case of conflict
      });
    }
    ...
    const newState: BidirectionalSyncState = {
      ...
      localHash,
      remoteHash,
      pendingPush: localChanges.length > 0,
      pendingPull: remoteChanges.length > 0,
    };

    this.syncState.set(stateKey, newState);
  ...
  /**
   * Get sync status without modifying state
   */
  async getSyncStatus(
    localTokens: DesignTokens,
    _stateKey?: string
    ...
    // Get remote tokens for comparison
    const remoteTokens = await this.pullTokens();
    // Note: _stateKey would be used for tracking sync status in full implementation

    const localPaths = this.getTokenPaths(localTokens);
    const remotePaths = this.getTokenPaths(remoteTokens);

    const pendingPush = localPaths.filter(p => !remotePaths.includes(p));
    const pendingPull = remotePaths.filter(p => !localPaths.includes(p));
    ...
    return {
      inSync: pendingPush.length === 0 && pendingPull.length === 0 && conflicts.length === 0,
      pendingPush,
      pendingPull,
      conflicts,
    };
  ...
  private mergeTokens(
    ...
    const mergeRecursive = (localObj: any, remoteObj: any, path: string[] = []): void => {
        ...
        if (remoteValue && typeof remoteValue === 'object' && !Array.isArray(remoteValue)) {
          if ('value' in remoteValue) {
            ...
            if (localValue && typeof localValue === 'object' && 'value' in localValue) {
              ...
              if (onConflict) {
                const resolved = onConflict(pathStr, localValue, remoteValue);
                if (resolved) {
                  localObj[key] = resolved;
                }
              } else {
                // Default: use remote
  ...
  /**
   * Get changed paths between two token sets
   */
  private getChangedPaths(oldTokens: any, newTokens: any): string[] {
    const oldPaths = this.getTokenPaths(oldTokens);
    const newPaths = this.getTokenPaths(newTokens);

    const changed = new Set<string>();

    // Check for added/removed
    oldPaths.forEach(p => !newPaths.includes(p) && changed.add(p));
    newPaths.forEach(p => !oldPaths.includes(p) && changed.add(p));

    // Check for modified
    for (const path of oldPaths.filter(p => newPaths.includes(p))) {
      const oldValue = this.getTokenByPath(oldTokens, path);
      const newValue = this.getTokenByPath(newTokens, path);
  ...
  /**
   * Compute simple hash for token set
   */
